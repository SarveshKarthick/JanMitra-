...
# Vercel serverless SQLite in /tmp directory
DB_FILE = "/tmp/janmitra.db" if os.environ.get("VERCEL") else "./janmitra.db"
...
def seed_if_empty():
    ...
    if not db.query(User).first():
        u1 = User(name="Sarvesh Karthick", email="sarvesh@janmitra.gov.in", password_hash="dummy_hash", phone="9876543210", role="citizen", district="Chennai", trust_score=99, seva_credits_balance=150.0)
        u2 = User(name="Hari", email="hari@janmitra.gov.in", password_hash="dummy_hash", phone="9876543211", role="government", district="Madurai", trust_score=98, seva_credits_balance=210.0)
        u3 = User(name="Bavisiya", email="bavisiya@janmitra.gov.in", password_hash="dummy_hash", phone="9876543212", role="ngo", district="Coimbatore", trust_score=97, seva_credits_balance=180.0)
        u4 = User(name="Sanjaay", email="sanjaay@janmitra.gov.in", password_hash="dummy_hash", phone="9876543213", role="csr", district="Salem", trust_score=96, seva_credits_balance=320.0)
...
app = FastAPI(title="JanMitra National DPI Vercel API")
...
def root():
    return {
        "status": "online",
        "platform": "JanMitra National Digital Public Infrastructure",
        "deployed_on": "Vercel Serverless Python",
...
def get_ledger(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "transactions": [{
            ...
            "status": t.status,
            "date": t.created_at.strftime("%Y-%m-%d %H:%M") if t.created_at else "Today"
        } for t in txs]
    }
...
def simulate_earn(db: Session = Depends(get_db)):
    ...
    if user:
        ...
        tx = WalletTransaction(
            ...
            digilocker_tx_hash=f"0x{int(datetime.utcnow().timestamp())}",
            status="verified"
        )
        db.add(tx)
        db.commit()
